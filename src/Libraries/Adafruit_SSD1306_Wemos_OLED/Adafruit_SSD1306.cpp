...
void Adafruit_SSD1306::begin(TwoWire& wire, uint8_t vccstate, uint8_t i2caddr) {
  _vccstate = vccstate;
  _i2caddr = i2caddr;
  this->wire = &wire;
...
//     pinMode(cs, OUTPUT);
// #ifdef HAVE_PORTREG
//     csport      = portOutput(digitalPinToPort(cs));
//     cspinmask   = digitalPinToBitMask(cs);
//     dcport      = portOutput(digitalPinToPort(dc));
...
//       pinMode(sclk, OUTPUT);
// #ifdef HAVE_PORTREG
//       clkport     = portOutput(digitalPinToPort(sclk));
//       clkpinmask  = digitalPinToBitMask(sclk);
//       mosiport    = portOutput(digitalPinToPort(sid));
...
void Adafruit_SSD1306::setContrast(uint8_t value)
{
  if (_vccstate == SSD1306_EXTERNALVCC)
  {
    if (value = 0x9F)
...
void Adafruit_SSD1306::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
  ...
  // this display doesn't need ints for coordinates, use local byte s for faster juggling
   uint8_t y = __y;
   uint8_t h = __h;
